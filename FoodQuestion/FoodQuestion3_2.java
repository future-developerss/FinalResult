...
import java.awt.*;
public class FoodQuestion3_2 extends JFrame {
	JPanel p1, p2, bt_p3, bottom, contentPane;
	JLabel title, subtitle, q1;
	String username;
	RoundButton main;
	static String[] text= {"한식","일식","양식"};
	static RoundButton[] btn= new RoundButton[3];
	public String name;
    public FoodQuestion3_2(String msg) {
		name=msg;
		setTitle(name+"의 오늘은?");
		...
		q1 = new JLabel("Q3 지금 가장 생각나는 음식 종류는 무엇인가요?");
		q1.setFont(font1);
		for (int i=0; i < 3; i++) {
	...
	private class MyActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			if (b.getText().equals("이전으로")){
				//이전페이지로 이동
				new FoodQuestion2(FoodQuestion3_2.this.name);
				FoodQuestion3_2.this.setVisible(false);
			}
		}
	}
	...
	private class MyActionListener_R implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			if (b.getText().equals("한식")){
				new FoodResult2(FoodQuestion3_2.this.name);
				FoodQuestion3_2.this.setVisible(false);
			}else if (b.getText().equals("일식")){
				new FoodResult4(FoodQuestion3_2.this.name);
				FoodQuestion3_2.this.setVisible(false);
			}else if (b.getText().equals("양식")){
				new FoodResult5(FoodQuestion3_2.this.name);
				FoodQuestion3_2.this.setVisible(false);
			}
		}
	}
