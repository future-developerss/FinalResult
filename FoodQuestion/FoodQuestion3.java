...
import java.awt.*;
public class FoodQuestion3 extends JFrame {
	...
	public String name;
    public FoodQuestion3(String msg) {
		name=msg;
		setTitle(name+"의 오늘은?");
		...
		q1 = new JLabel("Q1 지금 가장 생각나는 음식 종류는 무엇인가요?");
		q1.setFont(font1);
		for (int i=0; i < 3; i++) {
	...
	private class MyActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			if (b.getText().equals("이전으로")){
				//이전페이지로 이동
				new FoodQuestion2(FoodQuestion3.this.name);
				FoodQuestion3.this.setVisible(false);
			}
		}
	}
	...
	private class MyActionListener_R implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			if (b.getText().equals("한식")){
				new FoodResult1(FoodQuestion3.this.name);
				FoodQuestion3.this.setVisible(false);
			}else if (b.getText().equals("일식")){
				new FoodResult3(FoodQuestion3.this.name);
				FoodQuestion3.this.setVisible(false);
			}else if (b.getText().equals("양식")){
				new FoodResult6(FoodQuestion3.this.name);
				FoodQuestion3.this.setVisible(false);
			}
		}
	}
